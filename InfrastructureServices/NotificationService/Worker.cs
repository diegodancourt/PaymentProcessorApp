...
public class Worker(
    ...
    ICustomerApiClient customerApiClient) : BackgroundService
{
    ...
    private async Task ConsumeMessageAsync(IConsumer<Ignore, string> consumer, CancellationToken stoppingToken)
    {
        try
        {
            ...
            // Fetch customer data from CustomerApi
            var customer = await customerApiClient.GetCustomerAsync(paymentStatus.CustomerId);

            if (customer == null)
            {
                logger.LogError(
                    "Failed to fetch customer data for CustomerId: {CustomerId}. Skipping notification for PaymentId: {PaymentId}",
                    paymentStatus.CustomerId,
                    paymentStatus.PaymentId);
                return;
            }

            await emailService.SendPaymentNotificationAsync(paymentStatus, customer);

            logger.LogInformation(
                "Payment notification sent successfully for PaymentId: {PaymentId} to {CustomerEmail}",
                paymentStatus.PaymentId,
                customer.Email);
        ...
        {
            logger.LogError(ex, "Error processing payment status notification");
        }
    }
}